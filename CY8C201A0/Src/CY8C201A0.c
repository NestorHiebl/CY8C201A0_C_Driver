...
HAL_StatusTypeDef CY8_send_command(I2C_HandleTypeDef *hi2c, uint8_t command_code) {
	...
	if (HAL_OK == retval)
	{
		HAL_Delay(CY8C201A0_COMMAND_WAIT_TIME_MS);
	}
...
HAL_StatusTypeDef CY8_init(	I2C_HandleTypeDef *hi2c,
	...
	{
		/* Sensor not responding to custom address, try and set it by appealing to general call address */
...
HAL_StatusTypeDef CY8_set_i2c_addr(I2C_HandleTypeDef *hi2c, uint8_t old_address, uint8_t new_address) {
	/* The new address must conform to I²C standards - use caution and validate before calling */
	if (new_address) {
		return HAL_ERROR;
	}
